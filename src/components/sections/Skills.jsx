...
function SkillPill({ label }) {
  return (
    <motion.span
      whileHover={{ scale: 1.05, borderColor: "rgba(0,255,136,0.4)", color: "rgba(255,255,255,0.9)" }}
  ...
  return (
    <TiltCard className={span}>
      <SpotlightCard 
        ...
        className="rounded-2xl p-6 border border-white/5 bg-[#121214] h-full"
      >
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="flex flex-wrap gap-2">
            {items.map((item) => <SkillPill key={item} label={item} />)}
          </div>
        </div>
      </SpotlightCard>
    </TiltCard>
  ...
  return (
    <section id="skills" ref={ref} className="py-32 relative">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          {[
            { title: "Languages", items: SKILLS.Languages, span: "lg:col-span-2", accent: "#00ff88", icon: "⌨️" },
            { title: "Dev Tools", items: SKILLS["Dev Tools"], span: "", accent: "#7c3aed", icon: "🔧" },
